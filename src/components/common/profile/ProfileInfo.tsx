...
import { MediaViewerOrigin, SettingsScreens } from '../../../types/index';

import { SERVICE_NOTIFICATIONS_USER_ID } from '../../../config.ts';
import { formatPhoneNumber } from '../../../util/phoneNumber';
import {
  getMainUsername, getUserStatus, isAnonymousForwardsChat, isChatChannel, isSystemBot, isUserOnline,
...
const ProfileInfo = ({
  ...
  const handleOwnUsernameClick = useLastCallback(() => {
  ...
  function renderStatus() {
    ...
    if (user) {
      ...
      if (isForSettings && (user.phoneNumber || mainUsername)) {
        return (
          <div
            className={buildClassName(styles.status, 'status')}
            dir="auto"
          ...
          >
            <span className={styles.userStatus} dir="auto">
              ...
              {user.phoneNumber && mainUsername && ' • '}
              {mainUsername && (
                <span
                  role="button"
                  tabIndex={0}
                  onClick={handleOwnUsernameClick}
